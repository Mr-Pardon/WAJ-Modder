...
def finalize(input_path: Path, output_path: Path, overlay_frame: bool) -> None:
    assets = skill_root() / "assets"
    mask = Image.open(assets / "cardpack-cover-silhouette-300x440.png").convert("RGBA").resize(
        SIZE, Image.Resampling.LANCZOS
    )
    image = cover_crop(Image.open(input_path))
...
def main() -> int:
    ...
    finalize(args.input, args.output, args.overlay_frame)
    return 0
